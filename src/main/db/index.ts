...
const DEFAULT_ACHIEVEMENTS = [
  ...
  { type: 'review_flow', title: '今日一条龙', description: '完成1天统一复习', icon: '🧭', threshold: 1 },
  { type: 'review_flow', title: '坚持不断档', description: '完成7天统一复习', icon: '🎉', threshold: 7 },
  { type: 'review_flow', title: '复习工作台熟手', description: '完成30天统一复习', icon: '🛰', threshold: 30 },
] as const;

export function initAchievements() {
  const existingRows = sqlite.prepare('SELECT type, threshold FROM achievements').all() as {
    type: string;
    threshold: number;
  }[];
  const existingKeys = new Set(existingRows.map((row) => `${row.type}:${row.threshold}`));
  ...
  for (const achievement of DEFAULT_ACHIEVEMENTS) {
    const key = `${achievement.type}:${achievement.threshold}`;
